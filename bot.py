...
history_cau = [] # Lưu: {"res": "T/X", "val": số_nút}
current_matches = {}
...
# ================= SHOP DATA =================
SHOP_ITEMS = {
    "1": {"name": "Danh hiệu: Đại Gia", "price": 50000},
    "2": {"name": "Thẻ Đổi Màu Tên", "price": 20000}
}
...
async def auto_update_matches():
    ...
    try:
        ...
        for mid, m in current_matches.items():
            ...
            embed = discord.Embed(title="🏆 THÔNG TIN TRẬN ĐẤU 🏆", color=0x2b2d31)
            embed.description = "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            ...
            embed.set_footer(text=f"!cuoc {mid} <chu/khach> <số_tiền>")
...
async def taixiu(ctx, side: str, amount: int):
    ...
    query_db("INSERT INTO bets (user_id, match_id, amount, team, status) VALUES (?, 'TAI_XIU', ?, ?, ?)", 
...
async def cau(ctx):
    ...
    chart = "".join([f"{e['res']}({e['val']:02d}) " for e in history_cau[-10:]])
    
    graph = ""
    for i in range(18, 2, -3):
        row = f"{i:02d} ┃"
        for e in history_cau[-15:]:
            row += " ● " if abs(e['val'] - i) < 2 else "   "
        graph += row + "\n"
...
async def cuoc(ctx, match_id: str, side: str, amount: int):
    ...
    query_db("UPDATE users SET coins = coins - ? WHERE user_id = ?", (amount, ctx.author.id))
    query_db("INSERT INTO bets (user_id, match_id, amount, team, status) VALUES (?, ?, ?, ?, 'PENDING')", 
...
async def update_leaderboard():
    ...
    embed.add_field(name="🏆 DANH SÁCH CAO THỦ", value=lb_text or "Đang cập nhật...", inline=False)
...
async def vi(ctx):
    ...
    embed.description = f"👤 **Người dùng:** {ctx.author.mention}\n💰 **Số dư:** `{coins:,.0f}` Cash"
